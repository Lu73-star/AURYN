...
import 'package:auryn/auryn_core/personality/personality_profile.dart';
import 'package:auryn/auryn_core/personality/personality_traits.dart';
import 'package:auryn/auryn_core/personality/personality_events.dart';
import 'package:auryn/auryn_core/emotion/emotion_state.dart';
import 'package:auryn/auryn_core/personality/behavior_shaping.dart';
...
class PersonaManager {
  ...
  Future<void> initialize({
    ...
    if (_isInitialized) {
      AurynLogger().info('Already initialized', tag: 'PersonaManager');
      return;
    }
    ...
    _isInitialized = true;
    AurynLogger().info('Initialized with profile: ${_currentProfile?.name}', tag: 'PersonaManager');
  }
  ...
  bool removeProfile(String id) {
    ...
    if (_currentProfile?.id == id) {
      AurynLogger().warning('Cannot remove active profile', tag: 'PersonaManager');
      return false;
    }
  ...
  Future<void> switchProfile(String profileId, {String reason = 'manual'}) async {
    ...
    if (_currentProfile?.id == profileId) {
      AurynLogger().debug('Already using profile: $profileId', tag: 'PersonaManager');
      return;
    }
    ...
    AurynLogger().info('Switched to profile: ${newProfile.name}', tag: 'PersonaManager');
  }
  ...
  Future<void> _saveToStorage() async {
    ...
    try {
      ...
      AurynLogger().debug('Persistence enabled but Hive storage not yet implemented', tag: 'PersonaManager');
      ...
      AurynLogger().error('Error saving to storage', tag: 'PersonaManager', error: e);
    }
  }
  ...
  Future<void> _loadFromStorage() async {
    ...
    try {
      ...
      AurynLogger().error('Error loading from storage', tag: 'PersonaManager', error: e);
    }
  }
