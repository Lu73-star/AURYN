  ...
  void notifyStateChange(EmotionState previous, EmotionState current) {
    ...
    for (final callback in _onStateChangeCallbacks) {
      try {
        ...
        AurynLogger().error('Error in emotion state change callback', tag: 'EmotionHooks', error: e);
      }
    }
  ...
  void _notifyMoodChange(String previousMood, String newMood) {
    for (final callback in _onMoodChangeCallbacks) {
      try {
        ...
        AurynLogger().error('Error in mood change callback', tag: 'EmotionHooks', error: e);
      }
    }
  }
  ...
  void _notifyHighIntensity(EmotionState state) {
    for (final callback in _onHighIntensityCallbacks) {
      try {
        ...
        AurynLogger().error('Error in high intensity callback', tag: 'EmotionHooks', error: e);
      }
    }
  }
  ...
  void _notifyPositiveEmotion(EmotionState previous, EmotionState current) {
    for (final callback in _onPositiveCallbacks) {
      try {
        ...
        AurynLogger().error('Error in positive emotion callback', tag: 'EmotionHooks', error: e);
      }
    }
  }
  ...
  void _notifyNegativeEmotion(EmotionState previous, EmotionState current) {
    for (final callback in _onNegativeCallbacks) {
      try {
        ...
        AurynLogger().error('Error in negative emotion callback', tag: 'EmotionHooks', error: e);
      }
    }
  }
...
class EmotionHookPresets {
  ...
  static EmotionChangeCallback get loggingHook {
    return (previous, current) {
      AurynLogger().debug(
        'Mudança: ${previous.mood} -> ${current.mood} (intensity: ${current.intensity}, valence: ${current.valence})',
        tag: 'EmotionLog',
      );
    };
  }
  ...
  static HighIntensityCallback get dramaticChangeHook {
    return (state) {
      AurynLogger().info(
        'Estado emocional intenso detectado: ${state.mood} (intensity: ${state.intensity})',
        tag: 'EmotionAlert',
      );
    };
  }
  ...
  static MoodChangeCallback get moodTrackingHook {
    return (previousMood, newMood) {
      AurynLogger().debug('Humor mudou: $previousMood -> $newMood', tag: 'MoodTracker');
    };
  }
  ...
  static EmotionChangeCallback get positiveEmotionHook {
    return (previous, current) {
      AurynLogger().debug('Emoção positiva detectada: ${current.mood}', tag: 'PositiveVibes');
    };
  }
  ...
  static EmotionChangeCallback get supportiveHook {
    return (previous, current) {
      AurynLogger().info('Emoção negativa detectada: ${current.mood}. Sistema de suporte ativado.', tag: 'Support');
    };
  }
}
