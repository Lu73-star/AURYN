...
import 'package:auryn/auryn_core/memory/memory_entry.dart';
import 'package:auryn/auryn_core/memory/memory_scope.dart';
import 'package:auryn/auryn_core/memory/long_term_memory.dart';
import 'package:auryn/auryn_core/memory/episodic_memory.dart';
import 'package:auryn/auryn_core/memory/memory_traits.dart';
import 'package:auryn/auryn_core/memory/memory_expiration.dart';
...
class MemoryManager {
  ...
  Future<void> initialize({
    ...
    if (_isInitialized) {
      AurynLogger().info('Já inicializado', tag: 'MemoryManager');
      return;
    }
    ...
    try {
      ...
      final activeCount = await _longTerm.countActive();
      final traitCount = _traits.getAllTraits().length;
      AurynLogger().info(
        'Sistema de memória inicializado - Episódica: $episodicSize, Longo prazo: $activeCount, Traços: $traitCount',
        tag: 'MemoryManager',
      );
    } catch (e) {
      AurynLogger().error('Erro ao inicializar', tag: 'MemoryManager', error: e);
      rethrow;
    }
  }
  ...
  Future<void> close() async {
    ...
    AurynLogger().info('Sistema de memória fechado', tag: 'MemoryManager');
  }
  ...
  Future<void> storeInteraction({
    ...
    AurynLogger().debug('Interação armazenada: $userInput', tag: 'MemoryManager');
  }
  ...
  Future<void> retrainTraits() async {
    ...
    AurynLogger().info('Traços re-treinados com ${memories.length} memórias', tag: 'MemoryManager');
  }
  ...
  Future<void> clearAll() async {
    ...
    AurynLogger().info('Todas as memórias limpas', tag: 'MemoryManager');
  }
