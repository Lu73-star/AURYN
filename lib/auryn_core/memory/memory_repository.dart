...
import 'package:auryn/auryn_core/memory/memory_serializer.dart';
...
class MemoryRepository {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _isInitialized = true;
      AurynLogger().info('Inicializado com ${_box!.length} memórias', tag: 'MemoryRepository');
    } catch (e) {
      AurynLogger().error('Erro ao inicializar', tag: 'MemoryRepository', error: e);
      rethrow;
    }
  }
  ...
  Future<void> _rebuildIndex() async {
    ...
    if (savedIndex != null) {
      try {
        ...
        if (indexStats['total_indexed_entries'] == totalEntries) {
          ...
          AurynLogger().debug('Índice carregado do cache', tag: 'MemoryRepository');
          return;
        }
      } catch (e) {
        AurynLogger().warning('Erro ao carregar índice salvo', tag: 'MemoryRepository', error: e);
      }
    }
    ...
    AurynLogger().info('Índice reconstruído (${entries.length} entradas)', tag: 'MemoryRepository');
  }
  ...
  Future<void> _saveIndex() async {
    try {
      ...
      AurynLogger().error('Erro ao salvar índice', tag: 'MemoryRepository', error: e);
    }
  }
